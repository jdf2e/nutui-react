...
export const Toast: FunctionComponent<
  Partial<ToastProps> & Omit<React.HTMLAttributes<HTMLAnchorElement>, 'content'>
> & {
  show: typeof show
  hide: typeof hide
} = (props) => {
      ...
      {innerVisible ? (
        ...
        >
          <View className={`${classPrefix} ${classes}`} id={id}>
            ...
            >
              {hasIcon() ? (
                <View className={`${classPrefix}-icon-wrapper`}>{iconName()}</View>
              ) : null}
              {title ? (
                <View className={`${classPrefix}-title`}>{title}</View>
              ) : null}
              <View className={`${classPrefix}-text`}>{content || msg}</View>
            </View>
          </View>
        </Overlay>
      ) : null}
