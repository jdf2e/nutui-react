            ...
            visitor: {
                ImportDeclaration(path) {
                    if (path.node.source.value === pName && path.node.specifiers.length) {
                        ...
                        const realNamePackage = nodepath__namespace.normalize(cName);
                        let absolutePath = [];
                        try {
...
function harmony() {
    return process.env.TARO_ENV.indexOf('harmony') > -1;
}
function dynamic() {
    return process.env.TARO_ENV.indexOf('dynamic') > -1;
}
function babelComponentStyle(options = { cName: '@nutui/nutui-react-taro', pName: '' }) {
    return injectCSS(options);
}

module.exports = babelComponentStyle;
