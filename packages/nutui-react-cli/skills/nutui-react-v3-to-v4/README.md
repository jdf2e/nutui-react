...
把项目从 NutUI React v3（`@nutui/nutui-react` 3.x）升级到 v4（4.x）。
...
- **系统性完成 v3 → v4 升级**：按依赖升级 → 扫描盘点 → 逐组件改写 → 验证的流程，把项目从 `@nutui/nutui-react` 3.x 升到 4.x。
- **定位真正需要改的组件**：用 `nutui-react migrate 3 4 --apply ./src` 扫描项目，只列出项目实际用到、且有破坏性变更的组件，避免对全部组件盲目排查。
- **精确核对 API 差异**：用 `nutui-react diff 3 4 <Component>` 拿到两版 Props 的枚举值 / 默认值 / 类型的精确 `旧→新`，补齐迁移文档没写到的 API 级差异。
...
- `nutui-react migrate 3 4 [--component X] [--apply ./src]` —— 官方迁移文档（按组件切分 / 项目扫描）。
- `nutui-react diff 3 4 [Component]` —— 跨版本 Props 实测差异。
- `nutui-react doc <C>` / `token <C>` —— 完整文档与 Design Token 交叉核对。
