...
const App = () => {
  ...
  return (
    <ProductCard
        ...
        delivery = { state.delivery }
        shopName = { state.shopName }
        tag = { <div> 自定義店鋪介紹 </div>}> </ProductCard>
    );
};
export default App;
```
...
| 名稱 | 說明 | 默認值 |
| --- | --- | --- |
| \--nutui-productcard-font-size-0 | 卡片標簽的字體大小 | `$font-size-0` |
| \--nutui-productcard-left-border-radius | 卡片圖片的圓角大小 | `0` |
| \--nutui-productcard-left-background-color | 卡片圖片的背景顏色 | `inherit` |