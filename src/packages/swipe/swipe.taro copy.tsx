...
function preventDefault(event: any, stopPropagation?: boolean): void {
...
>((props, instanceRef) => {
  ...
  const wrapperStyle = {
    ...
    transform: `translate(${pxTransform(state.offset)}, 0)`,
  ...
  const setActionWidth = useCallback(
    ...
    [actionWidth, updateActionWidth]
  )

  const getWidth = useCallback(async () => {
    console.log('~~hanyu getWidth', rightWrapperRef.current)
