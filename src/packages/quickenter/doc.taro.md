...
```tsx
import { QuickEnter } from '@nutui/nutui-react-taro'
```
...
const Demo = () => {
  ...
  const options = [
    ...
    { title: '收藏', icon: <Star /> },
    { title: '关注', icon: <Heart /> },
    { title: '地址', icon: <Location /> },
  ]

  return (
    <>
      <Cell title="可滚动（10条数据）" onClick={() => setVisible(true)} />
      <QuickEnter
        visible={visible}
        options={options}
        onClose={() => setVisible(false)}
...
import React, { useState } from 'react'
import { QuickEnter, Cell } from '@nutui/nutui-react-taro'
import { Message, Home, Search, Cart, ArrowUp } from '@nutui/icons-react-taro'
