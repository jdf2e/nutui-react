/**
 * 响应式缩放系数（--scale-f）：结合京东站内原生桥与站外视口规则，
...
/** 将缩放值同步到 :root 的 --scale-f / --scale-font / --scale-icon */
function applyScaleCssVars(nextScale: number) {
  if (!canUseDom) return
  const rootStyle = document.documentElement.style
  rootStyle.setProperty('--scale-f', formatScaleValue(nextScale))
